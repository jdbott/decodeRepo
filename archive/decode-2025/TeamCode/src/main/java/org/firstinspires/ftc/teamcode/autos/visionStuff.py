...
def detect_pickleballs(frame, hsv_low, hsv_high, min_area, circ_thresh, close_iter, kern_size):
    ...
    for cnt in contours:
        ...
        if area < min_area or area > MAX_BALL_AREA:
            continue
...
def main():
    ...
    try:
        while True:
            ...
            # Detect pickleballs
            annotated, balls, mask = detect_pickleballs(
                frame, hsv_low, hsv_high, min_area, circ, close_iter, kern_size
            )
            ...
            hud_lines = [
                ...
                f"FOCAL: {FOCAL_LENGTH_PIXELS:.1f}px",
            ]
            if balls:
                best = balls[0]
                hud_lines.extend([
