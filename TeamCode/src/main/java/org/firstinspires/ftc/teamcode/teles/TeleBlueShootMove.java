...
import org.firstinspires.ftc.teamcode.ShootingCalc;
...
public class TeleBlueShootMove extends LinearOpMode {
    ...
    // =========================================================
    // You corrected: Y must be >= 69 (not X)
    private static final double AUTO_PREP_MIN_Y = 69.0;
    // Angle-from-+Y axis must be >= this (0 = straight along +Y)
    ...
    public void runOpMode() throws InterruptedException {
        ...
        follower.setStartingPose(new Pose(25, 90, Math.toRadians(180)));
        follower.updatePose();
    ...
    // =========================================================
    // Auto-prep chute condition
    //  - Y must be >= 69
    //  - angle from +Y axis must be >= 30 degrees
    //    (angle-from-+Y uses dx,dy so X affects the angle, as you intended)
    // =========================================================
    ...
    private void handleIntakeToggleAndFSM(Pose pose) {
        ...
        // Square toggles intake ON/OFF (same as before)
    ...
    private void handleTurretTrackingAndControl(Pose pose) {
        ...
        final double TURRET_MAX_DEG = 160.0;
        ...
        double odomTurretDeg = normalize180(angleToTargetRobotDeg + TURRET_OFFSET_DEG);

        double turretCurrentDeg = turret.getCurrentAngle();

        // =========================
        // Robot velocity (for SOTM)
        // =========================
        Vector robotVelVec = follower.getVelocity();
        double speed = robotVelVec.getMagnitude();
        boolean moving = speed > 5; // TUNE

        Vector robotPosVec = new Vector(botX, botY);
        ...
        if (gamepad2.dpad_left && (gamepad2.left_trigger > 0.5)) {
            turret.setAngle(wrapIntoTurretWindow(
                    turret.getTargetAngle() - 10,
                    turretCurrentDeg, TURRET_MIN_DEG, TURRET_MAX_DEG));
            manualInput = true;
        } else if (gamepad2.dpad_left) {
            turret.setAngle(wrapIntoTurretWindow(
                    turret.getTargetAngle() - 1.0,
                    turretCurrentDeg, TURRET_MIN_DEG, TURRET_MAX_DEG));
            manualInput = true;
        }
        ...
        if (turretShootingActive) {
            ...
            if (moving) {
                // ===== SHOOT ON THE MOVE (PHYSICS-BASED ARC) =====
                double heightMeters = TARGET_HEIGHT_M - CAM_HEIGHT_M;

                // distance in meters (FIXED direction)
                ...
                desiredTurretDeg = normalize180(angleRobotDeg + TURRET_OFFSET_DEG);

                double shotSpeed = shotVec.getMagnitude();
                double shotSpeedMps = shotSpeed / 39.3701;

                rpmCmd = shotSpeedMps * 60 * 4.5; // TUNE (4.0–5.5)

            } else {
                // ===== NORMAL AIM =====
                desiredTurretDeg = odomTurretDeg;
                rpmCmd = computeFlywheelRpmCmd(pose);
            }
            ...
            if (gamepad2.right_trigger > 0.5) {
                ...
                if (!didTxAim) {
                    turret.setAngle(safeCmdDeg + turretOffset);
                }
            }
        ...
        // Safety rumble
        // =========================
        boolean outOfRange = (odomTurretDeg < -160.0) || (odomTurretDeg > 160.0);
        ...
        telemetry.addData("Speed", String.format(Locale.US, "%.2f", speed));
